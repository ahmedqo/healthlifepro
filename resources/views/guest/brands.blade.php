...
@section('content')
    ...
    <div class="my-6 lg:my-10">
        ...
        <section id="brands">
            <div class="container mx-auto p-4 flex flex-col gap-4">
                <ul class="grid grid-rows-1 grid-cols-2 lg:grid-cols-5 gap-4">
                    @forelse ($data as $row)
                        <li>
                            <a href="{{ route('views.guest.products', [
                                'brand' => $row->slug,
                            ]) }}"
                                class="w-full shadow-x-core rounded-x-huge border border-x-black border-opacity-10 aspect-square overflow-hidden p-4 flex items-center justify-center bg-x-white">
                                <img src="{{ $row->Image }}" alt="{{ $row->slug }}-image"
                                    class="aspect-video w-full block max-w-full max-h-full object-contain object-center transition-transform duration-150 hover:scale-150" />
                            </a>
                            <div class="p-2">
                    ...
                    @empty
                        <li class="col-span-2 lg:col-span-5">
