...
@section('content')
    <div class="flex flex-col gap-2">
            ...
            <div class="w-max ms-auto flex flex-wrap">
                ...
                <os-dropdown label="Columns" position="{{ Core::lang('ar') ? 'start' : 'end' }}" class="W160 sharp">
                    ...
                    <ul class="w-full flex flex-col p-4 gap-2">
                        ...
                        <li class="w-full">
                            <label class="flex flex-wrap gap-2 items-center">
                                <input data-type="column" data-for="message" type="checkbox"
                                    class="w-4 h-4 rounded-x-thin checked:accent-x-prime outline-2 outline-x-prime -outline-offset-2 focus:outline focus-within:outline">
                                <span class="text-base text-x-black">
                                    {{ __('Message') }}
                                </span>
        ...
        <div class="bg-x-white rounded-x-huge shadow-x-core">
            <form id="search" action="{{ route('views.requests.index') }}" method="GET"
                ...
                <table data-type="display" class="w-max min-w-full">
                    ...
                    <thead>
                        <tr>
                            ...
                            <td data-is="name" class="text-x-black text-sm font-x-thin py-2 px-4 w-32">
                                {{ __('Name') }}
                            </td>
                            <td data-is="email" class="text-x-black text-sm font-x-thin py-2 px-4 w-32">
                                {{ __('Email') }}
                            </td>
                            <td data-is="phone" class="text-x-black text-sm font-x-thin py-2 px-4 w-32">
                                {{ __('Phone') }}
                            </td>
                            <td data-is="quantity" class="text-x-black text-sm font-x-thin py-2 px-4 w-32 text-center">
                                {{ __('Quantity') }}
                            </td>
                            <td data-is="message" class="text-x-black text-sm font-x-thin py-2 px-4">
                                {{ __('Message') }}
                            </td>
                            <td data-is="actions"
                    ...
                    <tbody>
                        @forelse ($data as $row)
                            <tr class="border-t border-t-x-shade">
                                ...
                                <td data-is="name" class="text-x-black text-base py-2 px-4 w-32">
                                    {{ $row->name }}
                                </td>
                                <td data-is="email" class="text-x-black text-base py-2 px-4 w-32">
                                    {{ $row->email }}
                                </td>
                                <td data-is="phone" class="text-x-black text-base py-2 px-4 w-32">
                                    {{ $row->phone }}
                                </td>
                                <td data-is="quantity" class="text-x-black text-base py-2 px-4 w-32 text-center">
                                    {{ $row->Length() }}
                                </td>
                                <td data-is="message" class="text-x-black text-base py-2 px-4">
                                    {{ $row->message ?? '_' }}
                                </td>
                                <td data-is="actions" class="w-32 text-x-black text-base py-2 px-4 pe-6">
                                    @include('shared.core.action', [
                                        'scene' => route('views.quotations.store', [
                                            'target' => $row->id,
                                        ]),
                                        'clear' => route('actions.requests.clear', $row->id),
                                    ])
                                </td>
                            </tr>
                        @empty
