...
@section('content')
    <div class="flex flex-col gap-2">
            ...
            <div class="w-max ms-auto flex flex-wrap">
                ...
                <os-dropdown label="Columns" position="{{ Core::lang('ar') ? 'start' : 'end' }}" class="W160 sharp">
                    ...
                    <ul class="w-full flex flex-col p-4 gap-2">
                        ...
                        <li class="w-full">
                            <label class="flex flex-wrap gap-2 items-center">
                                <input {{ Core::lang('en') ? 'checked' : '' }} data-type="column" data-for="description_en"
                                    type="checkbox"
                                    class="w-4 h-4 rounded-x-thin checked:accent-x-prime outline-2 outline-x-prime -outline-offset-2 focus:outline focus-within:outline">
                                <span class="text-base text-x-black">
                                    {{ __('Description') }} (en)
                        ...
                        <li class="w-full">
                            <label class="flex flex-wrap gap-2 items-center">
                                <input {{ Core::lang('fr') ? 'checked' : '' }} data-type="column" data-for="description_fr"
                                    type="checkbox"
                                    class="w-4 h-4 rounded-x-thin checked:accent-x-prime outline-2 outline-x-prime -outline-offset-2 focus:outline focus-within:outline">
                                <span class="text-base text-x-black">
                                    {{ __('Description') }} (fr)
                        ...
                        <li class="w-full">
                            <label class="flex flex-wrap gap-2 items-center">
                                <input {{ Core::lang('ar') ? 'checked' : '' }} data-type="column" data-for="description_ar"
                                    type="checkbox"
                                    class="w-4 h-4 rounded-x-thin checked:accent-x-prime outline-2 outline-x-prime -outline-offset-2 focus:outline focus-within:outline">
                                <span class="text-base text-x-black">
                                    {{ __('Description') }} (ar)
                ...
                <a title="{{ __('Create') }}" href="{{ route('views.categories.store') }}"
                    class="block p-2 rounded-x-huge text-x-black outline-none !bg-opacity-10 hover:bg-x-black hover:text-x-prime focus:bg-x-black focus:text-x-prime focus-within:bg-x-black focus-within:text-x-prime">
                    <svg class="block w-6 h-6 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                        <path
        ...
        <div class="bg-x-white rounded-x-huge shadow-x-core">
            <form id="search" action="{{ route('views.categories.index') }}" method="GET"
                ...
                <table data-type="display" class="w-max min-w-full">
                    ...
                    <thead>
                        <tr>
                            ...
                            <td data-is="description_en" class="text-x-black text-sm font-x-thin py-2 px-4">
                                {{ __('Description') }} (en)
                            </td>
                            <td data-is="description_fr" class="text-x-black text-sm font-x-thin py-2 px-4">
                                {{ __('Description') }} (fr)
                            </td>
                            <td data-is="description_ar" class="text-x-black text-sm font-x-thin py-2 px-4">
                                {{ __('Description') }} (ar)
                            </td>
                            <td data-is="actions"
                    ...
                    <tbody>
                        @forelse ($data as $row)
                            <tr class="border-t border-t-x-shade">
                                ...
                                <td data-is="image" data-text="{{ $row->Image }}"
                                ...
                                <td data-is="description_en" class="text-x-black text-base py-2 px-4">
                                    {{ $row->description_en ?? '_' }}
                                </td>
                                <td data-is="description_fr" class="text-x-black text-base py-2 px-4">
                                    {{ $row->description_fr ?? '_' }}
                                </td>
                                <td data-is="description_ar" class="text-x-black text-base py-2 px-4">
                                    {{ $row->description_ar ?? '_' }}
                                </td>
