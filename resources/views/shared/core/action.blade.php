...
<div class="w-max h-full mx-auto flex items-center gap-[2px] justify-center rounded-x-thin overflow-hidden">
    @if ($scene)
        <a title="scene" href="{{ $scene }}"
            class="px-3 py-1 flex items-center justify-center rounded-sm text-x-white hover:text-x-black focus-within:text-x-black bg-slate-400 hover:bg-slate-200 focus-within:bg-slate-200 outline-none">
    ...
    @if ($print)
        <a title="print" href="{{ $print }}"
            class="px-3 py-1 flex items-center justify-center rounded-sm text-x-white hover:text-x-black focus-within:text-x-black bg-emerald-400 hover:bg-emerald-200 focus-within:bg-emerald-200 outline-none">
            <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                <path
    ...
    @if ($patch)
        <a title="patch" href="{{ $patch }}"
            class="px-3 py-1 flex items-center justify-center rounded-sm text-x-white hover:text-x-black focus-within:text-x-black bg-amber-400 hover:bg-amber-200 focus-within:bg-amber-200 outline-none">
            <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                <path
    ...
    @if ($clear)
        <form action="{{ $clear }}" method="POST">
            ...
            <button type="submit" title="clear"
                class="px-3 py-1 flex items-center justify-center rounded-sm text-x-white hover:text-x-black focus-within:text-x-black bg-red-400 hover:bg-red-200 focus-within:bg-red-200 outline-none">
                <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                    <path
