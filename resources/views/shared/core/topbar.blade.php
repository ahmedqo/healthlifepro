<os-topbar fixed position="space-between"
    class="bg-gradient-to-br rtl:bg-gradient-to-bl bg-x-core no-action shadow-x-core">
    <button id="sidebar-trigger"
        class="block p-2 rounded-x-huge text-x-white outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black">
    ...
    <div class="w-max flex flex-wrap gap-2">
        ...
        <os-dropdown label="Settings" position="{{ Core::lang('ar') ? 'start' : 'end' }}" class="W140">
            <button slot="trigger"
                class="block p-2 rounded-x-huge text-x-white outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black">
                <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                    <path
            ...
            <ul class="sys-colors w-full flex flex-col">
                <li class="w-full">
                    <a href="{{ route('views.profile.patch') }}"
                        class="w-full flex flex-wrap gap-2 p-2 text-start text-x-black items-center outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black {{ request()->routeIs('views.profile.patch') ? '!bg-x-black' : '' }}">
                        <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                            <path
                        ...
                        <span class="block flex-1 text-sm">{{ __('Profile') }}</span>
                    </a>
                </li>
                <li class="w-full">
                    <a href="{{ route('views.password.patch') }}"
                        class="w-full flex flex-wrap gap-2 p-2 text-start text-x-black items-center outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black {{ request()->routeIs('views.password.patch') ? '!bg-x-black' : '' }}">
                        <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                            <path
                        ...
                        <span class="block flex-1 text-sm">{{ __('Password') }}</span>
                    </a>
                </li>
                <li class="w-full">
                    <form action="{{ route('actions.close.index') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="w-full flex flex-wrap gap-2 p-2 text-start text-x-black items-center outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black">
                            <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                                <path
                            ...
                            <span class="block flex-1 text-sm">{{ __('Logout') }}</span>
                        </button>
                    </form>
                </li>
            </ul>
