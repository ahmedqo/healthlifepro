...
<body close>
    <os-wrapper class="bg-x-light isolate">
        <os-topbar transparent position="end" class="fixed top-0 left-0 right-0 w-full pointer-events-none">
            <os-dropdown label="Languages" position="{{ Core::lang('ar') ? 'start' : 'end' }}"
                class="W140 pointer-events-auto">
                <button slot="trigger"
                    class="block p-2 rounded-x-huge text-x-white lg:text-x-black outline-none !bg-opacity-10 hover:bg-x-black focus:bg-x-black focus-within:bg-x-black">
                    ...
                    <li class="w-full">
                        <a href="{{ route('actions.language.index', 'en') }}"
                            ...
                            <img src="{{ asset('lang/en.png') }}?v={{ env('APP_VERSION') }}"
                                class="block w-6 h-4 object-contain bg-red-500" />
