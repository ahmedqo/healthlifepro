...
function checkProps(self, name, value, names) {
    ...
    else self.getAttribute(name) !== String(value) && self.setAttribute(name, value);
}
...
OS.Component.Wrapper = (function() {
    ...
    return OS.Component({
        ...
        rules: {
            toggle() {
                const els = [];
                [...document.querySelectorAll([OS.$Selectors.Sidebar, OS.$Selectors.Modal, OS.$Selectors.Select, OS.$Selectors.Date, OS.$Selectors.Dropdown].join(", "))].map((el) => {
                    el.state.show && els.push(el.tagName.toLowerCase());
                });
                if (els.length) {
                    this.state.med = ![OS.$Selectors.Modal, OS.$Selectors.Sidebar].some(el => els.includes(el));
...
OS.Component.Date = (function() {
        ...
        * {
        ...
        ($ if @state.show $)
            [part="modal"] {
                ...
                animation: opacity-{{ @props.toggle ? "on" : "off" }} 200ms ease-in-out forwards;
            }
            ...
            [part="content"] {
                ...
                background: {{ @theme.colors("OS-WHITE") }};
                box-shadow: {{ @theme.colors("OS-BLACK", 20) }} 0px 2px 12px;
                animation: slide-{{ @props.toggle ? "on" : "off" }} 200ms ease-in-out forwards;
            }
                        ...
                        <ul ref="control" part="control">
                            ...
                            <li ref="display" part="display">
                                <button ref="month" part="month" role="trigger" 
                                    ...
                                    @click="{{ @rules.view }}" 
                                >
                                    {{ @rules.slice(@props.locales[@props.locale].Months[@state.info.month], @props.fullMonth) }}
                                </button>
                            ...
                            ($ range day into 0 to 6 $)
                                <li ref="day" part="day">{{ @rules.slice(@props.locales[@props.locale].Days[day], @props.fullDay) }}</li>
                            ($ endrange $)
                        </ul>
                    ($ endif $)
                        ...
                        ($ if ["Y", "M"].includes(@state.view) $)
                            ...
                            ($ range value into range[0] to range[1] $)
                                <button ref="date" part="date" role="{{ @state.view === 'Y' ? 'year' : 'month' }}-button" type="button" 
                                    ...
                                    ($ endif $)
                                >
                                    {{ @state.view === "Y" ? value : @props.locales[@props.locale].Months[value] }}
                                </button>
                            ($ endrange $)
                        ($ else $)
    ...
    return OS.Component({
        ...
        rules: {
            ...
            toggle(event) {
                if (this.props.disabled) this.props.toggle = false;
                else if (event.type === "click" || event.keyCode === 13) this.props.toggle = !this.props.toggle;
            },
            blur(event) {
                if (!this.root.contains(event.target) && event.target !== this && this.props.toggle) {
            ...
            created() {
                const formats = {
                    ...
                    M: (date, token) => {
                        ...
                        switch (token) {
                            case "MMM":
                                return this.props.locales[this.props.locale].Months[month].slice(0, 3);
                            case "MMMM":
                                return this.props.locales[this.props.locale].Months[month];
                            default:
                    ...
                    d: (date, token) => {
                        switch (token) {
                            case "ddd":
                                return this.props.locales[this.props.locale].Days[date.getDay()].slice(0, 3);
                            case "dddd":
                                return this.props.locales[this.props.locale].Days[date.getDay()];
                            default:
                    ...
                    S: (date, token) => {
                        ...
                        return formats.__.zeros(fractionalSeconds, token.length);
                    },
                };
                ...
                if (type === "props") {
                    ...
                    switch (name) {
                        ...
                        case "toggle":
                            ...
                            break;
                        case "disabled":
                            newValue && (this.props.toggle = false);
...
OS.Component.Select = (function() {
                ...
                <div ref="content" part="content" @click:propagation="{{ () => {} }}">
                    ...
                    ($ if @props.search $)
                        <div ref="tools" part="tools">
                            <input type="search" ref="search" part="search" @input="{{ @rules.search }}" placeholder="{{ @props.locales[@props.locale].Search }}" />
                        </div>
                    ($ endif $)
                    <div ref="items" part="items">
                        <slot />                   
    ...
    return OS.Component({
        ...
        rules: {
            ...
            toggle(event) {
                if (this.props.disabled) this.props.toggle = false;
                if (event.type === "click" || event.keyCode === 13) this.props.toggle = !this.props.toggle;
            },
            blur(event) {
                if (!this.root.contains(event.target) && event.target !== this && this.props.toggle) {
            ...
            search(event) {
                ...
                for (let item of this.querySelectorAll(OS.$Selectors.Option)) {
                    const phrase_int = item.innerText.toUpperCase().trim();
                    const phrase_val = item.value.toUpperCase().trim();
                    const phrase_txt = item.text.toUpperCase().trim();
                    for (const niddle of value.split(" ")) {
                        if (phrase_int.includes(niddle) || phrase_txt.includes(niddle) || phrase_val.includes(niddle)) item.style.display = "";
        ...
        rules: {
            ...
            input(event) {
                ...
                if (this.truty(this.props.target, [""])) {
                    const array = document.querySelectorAll(this.props.target);
                    for (let item of array) {
                        const phrase_txt = item.innerText.toUpperCase().trim();
                        for (const niddle of value.split(" ")) {
                            if (phrase_txt.includes(niddle)) item.style.display = "";
                            else item.style.display = "none";
                        }
                    }
                    this.emit("search", {
            ...
            <ul ref="content" part="content" @click:propagation="{{ () => {} }}">
                ($ if Array.isArray(@props.results) $)
                    ($ each result into @props.results $)
                        <li ref="item" part="item" tabindex="0" data-idx="{{ @loop.index }}" @click:propagation="{{ (event) => @rules.select(event, result) }}" @keydown:propagation="{{ (event) => @rules.select(event, result) }}">
                            {{ @props.display === "this" ? result : result[@props.display] }}
                        </li>
                    ($ endeach $)
                ($ endif $)
            </ul>
        ...
        rules: {
            ...
            select(event, result) {
                if (event.type === "click" || event.keyCode === 13) {
                    const current = result; //this.props.results[+event.target.dataset.idx];
                    this.props.value = this.props.display === "this" ? current : current[this.props.display];
                    this.state.toggle = false;
                    this.emit("select", {
                        data: current
                    });
...
OS.Component.Dropdown = (function() {
    ...
    return OS.Component({
        ...
        rules: {
            ...
            blur(event) {
                if (!this.root.contains(event.target) && this.props.toggle) {
                    this.rules.toggle({ type: "click" });
                }
            },
            pos() {
                ...
                if (type === "attrs")
                    switch (name) {
                        case "label":
                            ...
                            break;
                        case "toggle":
                            this.props.toggle = this.truty(newValue);
                            break;
                    }
...
OS.Component.Sidebar = (function() {
    const Style = /*css*/ `
        * {
        ...
        @keyframes slide-off {
            ...
            100% { transform: translateX({{ @props.dir === "rtl" ? 20 : -20 }}px); }
        }
        ...
        @keyframes slide-on {
            0% { transform: translateX({{ @props.dir === "rtl" ? 20 : -20 }}px); }
    ...
    return OS.Component({
        ...
        setup: {
            ...
            mounted() {
                this.props.dir = document.documentElement.dir;
            },
            removed() {
                window.removeEventListener("click", this.rules.blur);
            },
